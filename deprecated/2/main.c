...
vli_modAddC(p_result1, p_left, p_right, p_mod);
int i;
for(i=0;i<4;i++) {
...
vli_modSubC(p_result1, p_left, p_right, p_mod);
int i;
for(i=0;i<4;i++) {
	if(p_result1[i]!=p_resultG[i])
  {
...
static void vli_modMult(uint64_t *p_result, uint64_t *p_left, uint64_t *p_right, uint64_t *p_mod)
{
 ...
 //vli_mult(l_productC, p_left, p_right);
    ...
    /////////////
    int flag = 0;
    for(int k = 0; k < 2 * NUM_ECC_DIGITS; k++){
...
int main(void) {
	...
	uint64_t result[2*NUM_ECC_DIGITS];

    const int LIST_SIZE = 32;
    uint64_t *A = (uint64_t*)malloc(sizeof(uint64_t)*LIST_SIZE);
    uint64_t *B = (uint64_t*)malloc(sizeof(uint64_t)*LIST_SIZE);
  ...
  p_right[3] = 0;*/
	...
	//vli_modAdd2(result, A, B, curve_p);
	vli_modAdd(result, p_left, p_right, curve_p);
	vli_modMult_fast(result, p_left, p_right);
  ...
  //vli_modAdd(p_left, p_left, p_right, curve_p);
